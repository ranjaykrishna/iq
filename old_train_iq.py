...
def create_model(args, vocab, embedding=None):
    ...
    vqg = IQ(False, vocab, len(vocab), args.max_length, args.hidden_size,
             args.num_categories,
...
def sample_for_each_category(vqg, image, args):
    ...
    if args.no_category_space:
        return None
    categories = torch.LongTensor(list(range(args.num_categories)))
    if torch.cuda.is_available():
        categories = categories.cuda()
...
def train(args):
    ...
    gen_optimizer = torch.optim.Adam(vqg.parameters(), lr=learning_rate)
    ...
    for epoch in range(args.num_epochs):
        for i, (images, questions, posteriors, answers,
                categories, qindices) in enumerate(data_loader):
            n_steps += 1
            ...
            if torch.cuda.is_available():
                images = images.cuda()
                questions = questions.cuda()
                posteriors = posteriors.cuda()
                answers = answers.cuda() # answers now includes categories
                # categories = categories.cuda()
                qindices = qindices.cuda()
            alengths = process_lengths(answers)
            ...
            outputs, z_logit, transformer_kl_loss, transformer_posteriors, reconstructed_image_features = vqg(images, answers, posteriors, questions)

            gen_loss = criterion(outputs.reshape(-1, outputs.size(-1)), questions.reshape(-1))
            total_loss = 0.0
            total_loss += args.lambda_gen * gen_loss
            gen_loss = gen_loss.item()

            # Variational loss.
            total_loss += args.lambda_z * transformer_kl_loss
            transformer_kl_loss = transformer_kl_loss.item()
